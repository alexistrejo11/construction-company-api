...
public class SubmitExpenseHandler {
    ...
    public Result<ExpenseResponse> handle(UserContext user, Long id) {
        ...
        Result<Void> auth = policy.requireProjectPermission(user, expense.getProject().getId(), Permission.EXPENSE_SUBMIT);
        if (!auth.isSuccess()) return Result.error(auth.getErrorType(), auth.getErrorMessage());
        Result<Void> transition = expense.submit();
