package io.github.alexisTrejo11.construction.company.modules.expense.features.correct;
...
public class CorrectExpenseHandler {
    ...
    public Result<ExpenseResponse> handle(UserContext user, Long id) {
        ...
        Result<Void> auth = policy.requireProjectPermission(user, expense.getProject().getId(), Permission.EXPENSE_CORRECT);
        if (!auth.isSuccess()) return Result.error(auth.getErrorType(), auth.getErrorMessage());
        Result<Void> transition = expense.returnToDraft();
