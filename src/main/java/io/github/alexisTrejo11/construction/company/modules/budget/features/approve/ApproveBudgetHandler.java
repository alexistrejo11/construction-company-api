...
public class ApproveBudgetHandler {
    ...
    public Result<BudgetResponse> handle(UserContext user, Long budgetId) {
        ...
        Result<Void> auth = authorizationPolicy.requireProjectPermission(user, budget.getProject().getId(), Permission.BUDGET_APPROVE);
        if (!auth.isSuccess()) return Result.error(auth.getErrorType(), auth.getErrorMessage());
        Result<Void> transition = budget.approve();
