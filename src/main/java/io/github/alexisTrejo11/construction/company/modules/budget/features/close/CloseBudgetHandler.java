...
public class CloseBudgetHandler {
    ...
    public Result<BudgetResponse> handle(UserContext user, Long budgetId) {
        ...
        Result<Void> auth = authorizationPolicy.requireProjectPermission(user, budget.getProject().getId(), Permission.BUDGET_CLOSE);
        if (!auth.isSuccess()) return Result.error(auth.getErrorType(), auth.getErrorMessage());
        Result<Void> transition = budget.close();
