...
- [ ] Confirm the `prod` profile validates the migrated schema.
...
- A fresh SQLite development database starts successfully. PostgreSQL migration and Hibernate validation remain to be verified against a running PostgreSQL instance.

## Phase 2: API and Response Foundation

Goal: establish stable transport contracts before implementing feature slices.

- [ ] Review `docs/api/endpoints.md` and mark MVP, deferred, and future endpoints.
- [ ] Confirm request, command/query, response, and permission names for each MVP slice.
- [ ] Confirm the API version and resource path conventions.
- [ ] Implement the response envelope from `docs/conventions/error-handling.md`.
- [ ] Remove the response `success` boolean.
- [ ] Ensure success responses omit `error` and failure responses omit `data`.
- [ ] Add the structured error response with `error_type`, `code`, `message`, and `details`.
- [ ] Add or finalize `traceId` handling at the envelope level.
- [ ] Add `FORBIDDEN` to the result taxonomy if required by authorization flows.
- [ ] Align `AppErrorResolver` with the final result-to-HTTP mapping.
- [ ] Align `GlobalExceptionHandler` and security error handlers with the same envelope.
- [ ] Define the pagination response and request contracts without exposing Spring `Page` or `Pageable` at the API boundary.
...
- [ ] Remove JWT bearer authentication from the active security flow.
- [ ] Configure stateful Spring Security session management.
- [ ] Define session cookie attributes and expiration.
- [ ] Define and enable the CSRF strategy for session-based authentication.
- [ ] Implement the active account statuses: `INVITED`, `ACTIVE`, `SUSPENDED`, and `DISABLED`.
- [ ] Implement invitation creation with assigned initial global roles.
- [ ] Generate single-use, expiring invitation tokens.
- [ ] Send invitation email through the configured mail channel.
- [ ] Implement invitation acceptance and password creation.
- [ ] Activate the user only after invitation acceptance completes.
- [ ] Implement login for active users only.
- [ ] Implement logout and session invalidation.
- [ ] Implement `/auth/me` and `/users/me`.
- [ ] Implement administrative user listing, profile updates, role updates, and status changes.
- [ ] Hash passwords with BCrypt and never return password fields in responses.
- [ ] Remove `UNKNOWN` as a valid persisted role in the target model.
- [ ] Replace legacy mutable auth DTOs where appropriate with records.
- [ ] Add authentication, invitation, and account-status verification.
...
- [ ] Define the initial `Permission` enum from implemented use cases.
- [ ] Define the initial global roles.
- [ ] Implement the immutable role-to-permission catalog.
- [ ] Implement immutable `UserContext` from the authenticated principal.
- [ ] Add the presentation adapter or argument resolver for the current user.
- [ ] Implement focused authorization policies.
- [ ] Implement active project-membership checks.
- [ ] Ensure domain entities and repositories do not read `SecurityContextHolder`.
- [ ] Configure coarse route-level security rules.
- [ ] Return expected forbidden outcomes through the application result flow.
- [ ] Add authorization checks for User, Invitation, Project, Phase, and Member use cases.
