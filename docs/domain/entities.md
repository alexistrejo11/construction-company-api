...
- Lifecycle: Unread and read, with delivery state handled separately if required.
- Relationships: Belongs to one user and may reference a related resource or event.
- Invariants: Recipient is valid; notification creation does not mutate the source business resource.
